...
export default function Queue() {
  ...
  const [step, setStep] = useState('service') // service | docs | identify | book | ticket
  ...
  const back = () => {
    if (step === 'service') return dispatch({ type: 'GO_HUB' })
    ...
    <div className="flex flex-1 flex-col">
      ...
      {step === 'ticket' && service && booking && (
        <Ticket
          ...
          identified={identified}
          booking={booking}
          onWelcomer={() => dispatch({ type: 'SET_CONCEPT', concept: 'welcomer' })}
          onAgain={() => {
            setServiceId(null)
            setStep('service')
          }}
          onHub={() => dispatch({ type: 'GO_HUB' })}
        />
      )}
    </div>
  )
...
function Ticket({ service, profile, identified, booking, onWelcomer, onAgain, onHub }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center px-5 py-7">
      ...
      {/* connect to the staff side */}
      <button
        onClick={onWelcomer}
        className="anim-fadeUp mt-6 w-full max-w-[320px] rounded-xl bg-ink py-3.5 text-[15px] font-bold text-white shadow-pop transition active:scale-[0.98]"
        style={{ animationDelay: '0.3s' }}
      >
        ดูสิ่งที่พนักงานเห็น (The Welcomer) →
      </button>
      <div className="mt-2.5 flex w-full max-w-[320px] gap-2.5">
        <button
          onClick={onAgain}
